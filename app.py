...
with st.sidebar:
    ...
    def _wp_settings_dialog() -> None:
        ...
        if _sites:
            st.caption("登録済みサイト")
            for _ws in _sites:
                _wc1, _wc2, _wc3 = st.columns([4, 1, 1])
                with _wc1:
        ...
        st.divider()
        st.caption("サイトを追加")
        _add_gen = st.session_state.get("wp_add_gen", 0)
        _wp_name = st.text_input("サイト名", placeholder="メインサイト", key=f"wp_add_name_{_add_gen}")
        _wp_url  = st.text_input("URL", placeholder="https://example.com", key=f"wp_add_url_{_add_gen}")
        _wp_user = st.text_input("ユーザー名", key=f"wp_add_user_{_add_gen}")
        _wp_pw   = st.text_input(
            "アプリケーションパスワード", type="password",
            placeholder="xxxx xxxx xxxx xxxx xxxx xxxx",
            key=f"wp_add_pw_{_add_gen}",
    ...
    if generate_clicked and has_input:
        ...
        with st.status("生成中...", expanded=True) as status:
            try:
                ...
                st.session_state.generation_done = True
                st.session_state.ui_mode = "edit"
                _gen_stripped, _ = _strip_images_for_edit(result)
                st.session_state["manual_edit_area"] = _gen_stripped
                _gen_secs = re.findall(r'^## (.+)$', result, flags=re.MULTILINE)
                st.session_state["img_regen_target"] = _gen_secs[0] if _gen_secs else None
                st.session_state.pop("img_regen_prompt", None)
